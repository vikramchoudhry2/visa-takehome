...
SEMANTIC_RULES: tuple[SemanticRule, ...] = (
    ...
    SemanticRule(
        ...
        system_prompt=(
            ...
            "EMPTY-BIO BAR: For each `--- Attendee row N ---` block, if the line after `Bio:` "
            "is not the literal `(empty)` and contains any non-whitespace text, you MUST NOT "
            "emit a finding that the Bio cell is empty, missing, or absent for that attendee.\n"
